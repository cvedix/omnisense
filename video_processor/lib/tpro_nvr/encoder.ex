defmodule TProNVR.AV.Encoder do
  @moduledoc false

  alias TProNVR.AV.VideoProcessor.NIF

  @type t :: reference()

  @type codec :: :h264 | :h265 | :mjpeg
  ...
  def new(codec, opts) when codec in [:h264, :h265, :mjpeg] do
  ...
  @spec encode(t(), TProNVR.AV.Frame.t()) :: [TProNVR.AV.Packet.t()]
  ...
  Flush the encoder.
  """
  @spec flush(t()) :: [TProNVR.AV.Packet.t()]
  ...
  defp to_packets(result) do
    Enum.map(result, fn {data, dts, pts, keyframe?} ->
      %TProNVR.AV.Packet{data: data, dts: dts, pts: pts, keyframe?: keyframe?}
    end)
  end
end
