# Deploy `cvr` using docker compose

In this guide, we'll deploy `cvr` using `docker compose`. We provide a `docker-compose.yml` and `.env` environment variables files.
...
```bash
docker pull ghcr.io/evercam/cvr:latest
```
...
* Make the `cvr` the owner of the */data* folder
  ```bash
  sudo chown -R nobody:nogroup /media/hdd/cvr
  ```
