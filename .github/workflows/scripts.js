...
async function handleNervesBuildCommand({ github, context, core }) {
  ...
  await postComment({
    ...
    body: `
      ...
      This may take up to 15 minutes. You can check the progress on the [Github Actions page](https://github.com/evercam/cvr/actions) 
    `,
  })
}
...
async function handleNervesBuildResult({ github, context, version, target, result }) {
  ...
  if (result === "success") {
    body = `
      ✅ Build succeeded!
      Version: **${version}** for target: **${formatTarget(target)}** can be found on [NervesHub](https://manage.nervescloud.com/org/Evercam/cvr_fw/firmware).
    `
  } else {
...
async function validateTarget({ github, context, core, target }) {
  const validTargets = ["cvr_rpi4", "cvr_rpi5", "giraffe"]
