...
config :tpro_nvr, TProNVR.Repo,
  database: Path.expand("../cvr_dev.db", Path.dirname(__ENV__.file)),
  pool_size: 5,
  stacktrace: true,
...
config :tpro_nvr,
  ...
  storage_path: Path.expand("../data", Path.dirname(__ENV__.file)),
  run_pipelines: true
...
config :tpro_nvr, TProNVRWeb.Endpoint,
  live_reload: [
    patterns: [
      ...
      ~r"priv/gettext/.*(po)$",
      ~r"lib/cvr_web/(controllers|live|components)/.*(ex|heex)$"
    ]
  ]

# Enable dev routes for dashboard and mailbox
config :tpro_nvr, dev_routes: true
