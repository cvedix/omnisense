defmodule CVR.RecordingsFixtures do
  @moduledoc """
  This module defines test helpers for creating
  entities via the `CVR.Recordings` context.
  """
  ...
  def recording_fixture(device, attrs \\ %{}) do
    {:ok, recording, _run} =
      ...
      |> tap(
        &File.mkdir_p!(
          Path.dirname(CVR.Recordings.recording_path(device, attrs[:stream] || :high, &1))
        )
      )
      |> then(&CVR.Recordings.create(device, attrs[:run] || run_fixture(device), &1))
