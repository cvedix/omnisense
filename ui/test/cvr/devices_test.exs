defmodule CVR.DevicesTest do
  use CVR.DataCase

  alias CVR.Devices
  alias CVR.Model.{Device, Recording, Run}

  import CVR.DevicesFixtures
  ...
  describe "delete/1" do
    ...
    test "delete device", %{device: device} do
      ...
      assert CVR.Repo.get(Device, device.id)
      assert CVR.Repo.aggregate(Recording.with_device(device.id), :count) > 0
      assert CVR.Repo.aggregate(Run.with_device(device.id), :count) > 0

      assert :ok == Devices.delete(device)

      refute CVR.Repo.get(Device, device.id)
      assert CVR.Repo.aggregate(Recording.with_device(device.id), :count) == 0
      assert CVR.Repo.aggregate(Run.with_device(device.id), :count) == 0
    end
  end
  ...
  test "get recordings dir", ctx do
    ...
    assert Path.join([ctx.tmp_dir, "cvr", device.id]) == Device.base_dir(device)

    assert Path.join([ctx.tmp_dir, "cvr", device.id, "hi_quality"]) ==
             Device.recording_dir(device)

    assert Path.join([ctx.tmp_dir, "cvr", device.id, "lo_quality"]) ==
             Device.recording_dir(device, :low)

    assert Path.join([ctx.tmp_dir, "cvr", device.id, "bif"]) == Device.bif_dir(device)
  end
end
