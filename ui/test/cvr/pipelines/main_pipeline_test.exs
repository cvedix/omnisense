defmodule CVR.Pipelines.MainPipelineTest do
  @moduledoc false
  ...
  alias CVR.{Devices, Utils}
  alias CVR.Pipeline.Track
  alias CVR.Pipelines.Main, as: MainPipeline
  ...
  test "main pipeline", %{device: device} do
    ...
    socket_server =
      if elem(:os.type(), 0) == :unix do
        ...
        assert {:ok, socket_server} = CVR.UnixSocketServer.start(device: device)
        socket_server
      end
  ...
  defp prepare_pipeline(device) do
    options = [
      module: CVR.Pipelines.Main,
      custom_args: [device: device],
      name: CVR.Utils.pipeline_name(device)
    ]
