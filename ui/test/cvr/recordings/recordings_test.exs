defmodule CVR.RecordingTest do
  @moduledoc false

  use CVR.DataCase

  import CVR.DevicesFixtures
  import CVR.RecordingsFixtures

  alias CVR.Model.{Recording, Run}
  alias CVR.Recordings
  ...
  test "delete multiple recordings", %{device: device} do
    ...
    total_recordings = CVR.Repo.aggregate(Recording, :count)
    total_runs = CVR.Repo.aggregate(Run, :count)

    assert Recordings.delete_oldest_recordings(device, 30) == :ok

    assert CVR.Repo.aggregate(Recording, :count) == total_recordings - 59
    assert CVR.Repo.aggregate(Run, :count) == total_runs
    ...
    assert CVR.Repo.aggregate(Recording, :count) == total_recordings - 89
    assert CVR.Repo.aggregate(Run, :count) == total_runs - 2
  ...
  test "runs summary", ctx do
    ...
    assert CVR.Recordings.runs_summary(15 * 60) == expected
    ...
    assert %{
           ...
           } = CVR.Recordings.runs_summary(600)
  end
