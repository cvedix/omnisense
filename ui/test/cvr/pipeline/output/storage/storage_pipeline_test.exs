defmodule CVR.Pipeline.Output.Storage.StoragePipelineTest do
  @moduledoc false
  ...
  alias CVR.Pipeline.Output.Storage
  ...
  defmodule Timestamper do
    ...
    def handle_buffer(:input, buffer, _ctx, state) do
      # flatten metadata to match the format created by `cvr_rtsp`
      metadata =
  ...
  defp perform_test(device, fixture) do
    ...
    assert {:ok, {recordings, _meta}} = CVR.Recordings.list()
    ...
    for recording <- recordings do
      assert CVR.Recordings.recording_path(device, recording) |> File.exists?()
    end
  end
