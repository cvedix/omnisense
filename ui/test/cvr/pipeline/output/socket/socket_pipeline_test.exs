defmodule CVR.Pipeline.Output.SocketPipelineTest do
  @moduledoc false
  ...
  alias CVR.{UnixSocketServer, Utils}
  ...
  if :os.type() |> elem(0) == :unix do
    ...
    defp start_pipeline do
      spec = [
        child(:source, CVR.Support.TestSource)
        |> child(:parser, %Membrane.H264.Parser{
          generate_best_effort_timestamps: %{framerate: {1, 30}}
        })
        |> child(:sink, CVR.Pipeline.Output.Socket)
      ]
