defmodule CVRWeb.DeviceListLiveTest do
  @moduledoc false
  use CVRWeb.ConnCase

  import CVR.{AccountsFixtures, DevicesFixtures}
  ...
  describe "Start/Stop device recording" do
    ...
    test "Stop recording", %{conn: conn, tmp_dir: tmp_dir} do
      ...
      assert CVR.Devices.get!(device.id).state == :stopped
    end
    ...
    test "Start recording", %{conn: conn, tmp_dir: tmp_dir} do
      ...
      assert html =~ "Stop recording"
      assert CVR.Devices.get!(device.id).state == :recording
    end
  end
end
