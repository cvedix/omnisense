defmodule CVRWeb.Api.EventControllerTest do
  @moduledoc false
  use CVRWeb.ConnCase

  import CVR.{AccountsFixtures, DevicesFixtures, EventsFixtures}

  alias CVR.Model.Device
  ...
  describe "POST /api/device/:device/events" do
    setup do
      %{token: CVR.Accounts.generate_webhook_token(user_fixture())}
    end
    ...
    test "create a new generic event", %{conn: conn, device: device, token: token} do
      ...
      {:ok, {[event], _}} = CVR.Events.list_events(%{})
  ...
  defp matches(api_event, %CVR.Events.Event{} = event) do
