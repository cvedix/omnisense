...
CVR UI is the public-facing web app + REST API for the CVR project.
...
* **[REST API](https://evercam.github.io/cvr/)** providing access to recordings; HLS streaming; devices: system status...etc
...
To run the dev server for CVR UI, there are two options
...
This will start the Phoenix server and run the pipeline defined in `ui/lib/cvr_web/application.ex`, using the config defined in `ui/config/runtime.exs`, which will, among other things:
...
If you don't have an IP camera, but need to debug cvr locally, you can setup an RTSP server locally, and stream the output from your webcam.
...
3. Once the stream is configured, go to **CVR UI -> Devices -> Add Device** and fill the form with mock data, while providing the RTSP stream URL created above ( **rtsp://localhost:8554/webcam** ) and leave the username / password fields empty
