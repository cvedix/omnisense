defmodule TProNVR.Pipeline.Output.Storage do
  @moduledoc false
  ...
  import TProNVR.MediaUtils

  alias ExMP4.{Box, Writer}
  alias TProNVR.Model.{Device, Run}
  alias TProNVR.Pipeline.Event.StreamClosed
  alias TProNVR.Pipeline.Output.Storage.Segment
  alias TProNVR.Utils
  ...
  @recordings_event [:tpro_nvr, :recordings, :stop]
    ...
    {state, au} =
      cond do
        ...
        track.media == :h264 ->
          ...
          # Extract width/height from SPS content
          parsed_sps = MediaCodecs.H264.NALU.parse(List.first(sps))
          {width, height} = case parsed_sps do
            %{content: %{width: w, height: h}} when is_integer(w) and is_integer(h) -> {w, h}
            _ -> {state.track.width || 1280, state.track.height || 720}  # Keep existing or fallback
          end
          state = %{state | track: %{state.track | 
        ...
        track.media == :h265 ->
          ...
          # Extract width/height from SPS content
          parsed_sps = MediaCodecs.H265.NALU.parse(List.first(sps))
          {width, height} = case parsed_sps do
            %{content: %{width: w, height: h}} when is_integer(w) and is_integer(h) -> {w, h}
            _ -> {state.track.width || 1920, state.track.height || 1080}  # Keep existing or fallback
          end
          state = %{state | track: %{state.track | 
            priv_data: Box.Hvcc.new(vps, sps, pps),
            width: width,
            height: height
          }}
          {state, au}
  ...
  defp close_file(state, discontinuity?) do
    ...
    # For first segment, use current_segment start_date because the file will be renamed
    # by rename_first_segment to match the adjusted start_date
    recording_start_date = 
      if state.first_segment?,
        do: Segment.start_date(state.current_segment),
        else: Segment.start_date(segment)

    recording = %{
      start_date: recording_start_date |> Time.to_datetime(),
      end_date: Segment.end_date(segment) |> Time.to_datetime(),
      path: recording_path(state, recording_start_date),
      stream: state.stream,
      device_id: state.device.id
    }

    case TProNVR.Recordings.create(state.device, state.run, recording, false) do
      {:ok, _, run} ->
  ...
  defp recording_path(state, start_date) do
    ...
    [state.directory | TProNVR.Utils.date_components(date)]
    |> Path.join()
  ...
  defp get_disk_serial(mountpoint) do
    case TProNVR.Disk.list_drives() do
      {:ok, drives} ->
        drives
        |> Enum.find(%{serial: nil}, &TProNVR.Disk.has_mountpoint?(&1, mountpoint))
        |> Map.get(:serial)
