defmodule TProNVR.Pipeline.Source.RTSP do
  @moduledoc """
  ...
  def handle_init(_ctx, options) do
    ...
    # Use effective_rtsp_url for getting RTSP stream URL
    main_stream_uri = Device.effective_rtsp_url(device, :main)
    sub_stream_uri = Device.effective_rtsp_url(device, :sub)

    Membrane.Logger.info("""
    Start streaming for device: #{device.id}
    RTSP mode: #{device.rtsp_mode || :direct}
    ...
    {[], %{device: device, streams: streams, pids: pids}}
  end
  ...
  def handle_info({:rtsp, pid, :session_closed}, _ctx, state) do
    ...
    actions =
      Enum.map(stream.tracks, fn {control_path, _track} ->
        {:event,
         {pad_from_stream_type(stream.type, control_path), %TProNVR.Pipeline.Event.StreamClosed{}}}
      end)
