defmodule TProNVR.Recordings do
  @moduledoc """
  ...
  alias TProNVR.{AV, Repo}
  alias TProNVR.Model.{Device, Recording, Run}
  ...
  def create(%Device{} = device, %Run{} = run, params, copy_file? \\ true) do
    ...
    # Validate device still exists in DB before inserting (SQLite FK errors are unrecoverable)
    unless Repo.get(Device, device.id) do
      {:error, :device_not_found}
    else
      with :ok <- copy_file(device, params, copy_file?) do
        run_changeset = Run.changeset(run, %{})

        Multi.new()
        |> Multi.insert(:run, run_changeset, on_conflict: {:replace_all_except, [:start_date]})
        |> Multi.insert(:recording, fn %{run: run} ->
          # Use explicit filename if provided (e.g. from GStreamer watcher)
          # Otherwise calculate from recording_path (Membrane convention)
          filename = params[:filename] || params["filename"] || (recording_path(device, params) |> Path.basename())
  ...
  def list(params \\ %{}, stream_type \\ :high) do
    ...
    |> TProNVR.Flop.validate_and_run(params, for: Recording)
  end
  ...
  def snapshot(device, recording, datetime, opts \\ []) do
    ...
    with {:ok, reader} <- ExMP4.Reader.new(path) do
      ...
      |> TProNVR.MediaUtils.decode_last(AV.Decoder.new(track.media, out_format: :yuvj420p))
      |> then(fn frame ->
        datetime =
          DateTime.add(
  ...
  def recording_path(device, stream_type \\ :high, recording) do
    # Use explicit filename if available (from recording struct or GStreamer)
    # Otherwise calculate from start_date (Membrane convention)
    # Use Map.get to support both structs and maps
    filename = Map.get(recording, :filename) || Map.get(recording, "filename") || 
               "#{DateTime.to_unix(recording.start_date, :microsecond)}.mp4"
    
    Path.join(
      [Device.recording_dir(device, stream_type) | TProNVR.Utils.date_components(recording.start_date)] ++
        [filename]
    )
  end
  ...
  def correct_run_dates(device, run, duration) do
    ...
    Enum.each(recs, fn rec ->
      ...
      new_rec = TProNVR.Repo.update!(changeset)

      File.rename!(
        TProNVR.Recordings.recording_path(device, rec),
        TProNVR.Recordings.recording_path(device, new_rec)
      )
    end)
  ...
  def subscribe_to_recording_events do
    PubSub.subscribe(TProNVR.PubSub, @recordings_topic)
  end
  ...
  defp broadcast_recordings_event(event) do
    PubSub.broadcast(TProNVR.PubSub, @recordings_topic, {event, nil})
  end
