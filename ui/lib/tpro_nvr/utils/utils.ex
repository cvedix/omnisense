defmodule TProNVR.Utils do
  ...
  def hls_dir(device_id \\ nil) do
    dir = Application.get_env(:tpro_nvr, :hls_directory)
  ...
  def unix_socket_path(device_id) do
    Path.join(@unix_socket_dir, "cvr.#{device_id}.sock")
  end
  ...
  def run_main_pipeline?, do: Application.get_env(:tpro_nvr, :run_pipelines, true)
  ...
  def writable(dest) do
    touch_file = Path.join(dest, ".cvr_check")
