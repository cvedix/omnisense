defmodule TProNVR.Hardware.Victron do
  @moduledoc """
  ...
  def handle_info(:report, state) do
    ...
    if state.data.pid in ["0xA389", "0xA38A", "0xA38B"],
      do: TProNVR.SystemStatus.set(:battery_monitor, data),
      else: TProNVR.SystemStatus.set(:solar_charger, data)
  ...
  def terminate(reason, state) do
    if pid = state.data.pid do
      ...
      if pid in ["0xA389", "0xA38A", "0xA38B"],
        do: TProNVR.SystemStatus.set(:battery_monitor, nil),
        else: TProNVR.SystemStatus.set(:solar_charger, nil)
    end
