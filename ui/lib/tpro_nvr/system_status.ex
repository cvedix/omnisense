defmodule TProNVR.SystemStatus do
  @moduledoc """
  ...
  def init(_options) do
    ...
    data =
      %{
        version: Application.spec(:tpro_nvr, :vsn) |> to_string(),
  ...
  def handle_call(:get, _from, state) do
    data = Map.put(state.data, :devices, TProNVR.Devices.summary())
    {:reply, data, state}
  end
  ...
  def handle_call({:get, key}, _from, state) do
    data =
      if key == :devices do
        Map.put(state.data, :devices, TProNVR.Devices.summary())
      else
  ...
  defp list_block_storages do
    ...
    case TProNVR.Disk.list_drives(major_number: [8, 179, 259]) do
      {:ok, blocks} -> blocks
      _ -> []
    end
  rescue
