defmodule TProNVR.Recordings.Concatenater do
  @moduledoc """
  ...
  @opaque t :: %__MODULE__{
            ...
            recordings: [TProNVR.Model.Recording.t()],
            current_recording: TProNVR.Model.Recording.t() | nil,
            reader: Reader.t() | nil,
            tracks: map()
          }
  ...
  defp open_next_file(%__MODULE__{recordings: [recording | recordings]} = state) do
    ...
    file_path = Recordings.recording_path(state.device, state.stream, recording)
    
    # Try to open the file - skip if it's incomplete/corrupt
    case safe_open_reader(file_path) do
      {:ok, reader} ->
        state = %__MODULE__{
          state
          | reader: reader,
        ...
        case safe_get_tracks(reader, state) do
          {:ok, tracks} ->
            offset =
              Map.values(tracks)
          ...
          {:error, reason} ->
            Logger.warning("[Concatenater] Skipping corrupt recording #{recording.id}: #{inspect(reason)}")
            # Skip to next recording
            open_next_file(%{state | recordings: recordings})
        end
        
      {:error, reason} ->
        Logger.warning("[Concatenater] Cannot open file #{file_path}: #{inspect(reason)}")
        # Skip to next recording
        open_next_file(%{state | recordings: recordings})
    end
  end
  
  # Safely open a reader, handling incomplete files
  defp safe_open_reader(file_path) do
    try do
      {:ok, Reader.new!(file_path)}
    rescue
      e -> {:error, Exception.message(e)}
    end
  end
  
  # Safely get tracks from reader, handling nil or invalid data
  defp safe_get_tracks(reader, state) do
    try do
      tracks = Reader.tracks(reader)
      
      if is_nil(tracks) or tracks == [] do
        {:error, :no_tracks}
      else
        video_tracks = 
          tracks
        ...
        if map_size(video_tracks) == 0 do
          {:error, :no_video_tracks}
        else
          {:ok, video_tracks}
        end
      end
