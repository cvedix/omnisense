defmodule TProNVR.Pipelines.Main do
  @moduledoc """
  ...
  The pipeline emits the following `:telemetry` events:
    * `[:tpro_nvr, :main_pipeline, :state]` - event emitted when the state of the device is updated.
    The event has no measurements and has the following metadata:
      ...
      * `old_state` - The old state of the device, (see `TProNVR.Model.Device.state()`)
      * `new_state` - The new state of the device, (see `TProNVR.Model.Device.state()`)

    * `[:tpro_nvr, :main_pipeline, :terminate]` - event emitted when the pipeline is about to be terminated.
  ...
  alias TProNVR.{Devices, Recordings, Utils}
  alias TProNVR.Elements.VideoStreamStatReporter
  alias TProNVR.Model.Device
  alias TProNVR.Pipeline.{Output, Source, StorageMonitor}

  @type encoding :: :H264 | :H265

  @event_prefix [:tpro_nvr, :main_pipeline]
  ...
  @spec get_tracks(Device.t()) :: %{atom() => [TProNVR.Pipeline.Track]}
  ...
  def handle_info({:new_socket, unix_socket}, ctx, state) do
    ...
    if Enum.member?(childs, :unix_socket) do
      ...
      spec = [
        source
        |> via_out(:push_output)
        |> child(:unix_socket, TProNVR.Pipeline.Output.Socket)
      ]
  ...
  defp build_device_spec(%{type: :file} = device) do
    [child(:file_source, %TProNVR.Pipeline.Source.File{device: device})]
  end
  ...
  defp build_main_stream_spec(state) do
    build_main_stream_storage_spec(state) ++
      [
        ...
        get_child(:tee)
        |> via_out(:push_output)
        |> child({:snapshooter, :main_stream}, TProNVR.Elements.CVSBufferer),
        get_child(:tee)
        |> via_out(:push_output)
  ...
  defp ice_servers do
    Application.get_env(:tpro_nvr, :ice_servers, "[]") |> Jason.decode(keys: :atoms)
  end
end
