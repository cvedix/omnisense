defmodule TProNVR.Devices.Onvif do
  @moduledoc false
  ...
  def auto_configure(%{manufacturer: manufacturer} = onvif_device)
      when manufacturer in ["TP-Link", "Tapo", "TP-LINK", "tp-link"] do
    Logger.info("Auto configure Tapo/TP-Link camera")

    %AutoConfig{}
    |> do_configure_tapo_profiles(onvif_device)
  end
  ...
  defp fetch_profiles(onvif_device) do
    with {:ok, profiles} <- Media2.get_profiles(onvif_device) do
      main_profile = Enum.find(profiles, &(&1.name == "cvr_main"))
      sub_profile = Enum.find(profiles, &(&1.name == "cvr_sub"))
  ...
  defp create_profiles(device) do
    with {:ok, sources} <- Media2.get_video_source_configurations(device),
         ...
         {:ok, main} <-
           do_create_profile(device, "cvr_main", hd(sources), Enum.at(configs, 0)),
         {:ok, sub} <- do_create_profile(device, "cvr_sub", hd(sources), Enum.at(configs, 1)) do
      {:ok, {main, sub}}
    end
  end
  ...
  # Tapo/TP-Link specific profile configuration
  # Tapo cameras use standard /stream1 (HD) and /stream2 (SD) RTSP paths
  defp do_configure_tapo_profiles(auto_config, onvif_device) do
    Logger.info("[Onvif] Configure Tapo/TP-Link profiles")

    # Try standard ONVIF configuration first
    case ExOnvif.Media2.get_profiles(onvif_device) do
      {:ok, profiles} when length(profiles) >= 2 ->
        auto_config
        |> do_configure_profile(onvif_device, Enum.at(profiles, 0), :main_stream)
        |> do_configure_profile(onvif_device, Enum.at(profiles, 1), :sub_stream)

      {:ok, [profile]} ->
        # Single profile - just configure main stream
        do_configure_profile(auto_config, onvif_device, profile, :main_stream)

      _error ->
        Logger.warning("[Onvif] Could not get Tapo profiles via ONVIF, manual RTSP config may be needed")
        auto_config
    end
  end

  @doc """
  Generate Tapo camera RTSP stream URL.

  ## Parameters
    - ip: Camera IP address
    - username: Camera account username (set via Tapo app)
    - password: Camera account password
    - stream: :main for HD stream1, :sub for SD stream2

  ## Example
      iex> TProNVR.Devices.Onvif.tapo_stream_url("192.168.1.100", "admin", "pass123", :main)
      "rtsp://admin:pass123@192.168.1.100:554/stream1"
  """
  @spec tapo_stream_url(String.t(), String.t(), String.t(), :main | :sub) :: String.t()
  def tapo_stream_url(ip, username, password, stream \\ :main) do
    stream_path = if stream == :main, do: "stream1", else: "stream2"
    "rtsp://#{URI.encode_www_form(username)}:#{URI.encode_www_form(password)}@#{ip}:554/#{stream_path}"
  end
