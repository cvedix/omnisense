defmodule TProNVR.Accounts.User do
  @moduledoc false
  ...
  def user_info_changeset(user, attrs, opts \\ []) do
    user
    |> cast(attrs, [:language, :first_name, :last_name, :timezone])
    |> validate_timezone()
    |> validate_user_language(opts)
    |> validate_user_full_name(opts)
  end
  ...
  """
  def valid_password?(%TProNVR.Accounts.User{hashed_password: hashed_password}, password)
  ...
  defp validate_timezone(changeset) do
    ...
    if timezone do
      if timezone in Tzdata.zone_list() do
        changeset
      else
        add_error(changeset, :timezone, "Invalid timezone")
      end
    else
      changeset
    end
  ...
  defp maybe_validate_unique_email(changeset, opts) do
    if Keyword.get(opts, :validate_email, true) do
      changeset
      |> unsafe_validate_unique(:email, TProNVR.Repo)
