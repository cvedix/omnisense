defmodule TProNVR.Elements.VideoStreamStatReporter do
  @moduledoc """
  ...
  @info_event [:tpro_nvr, :device, :stream, :info]
  ...
  @impl true
  def handle_event(:input, %TProNVR.Pipeline.Event.StreamClosed{}, _ctx, state) do
    {[], Map.merge(state, init_state())}
  end
  ...
  def handle_buffer(:input, buffer, _ctx, state) do
    ...
    state =
      if TProNVR.Utils.keyframe(buffer) do
        %{
          state
  ...
  def handle_tick(:report_stats, _ctx, state) do
    stats = %TProNVR.Pipeline.Track.Stat{
