defmodule TProNVRWeb.PromEx.SystemStatus do
  @moduledoc """
  ...
  defp solar_charger_metrics(poll_rate) do
    Polling.build(
      ...
      [
        last_value(
          "cvr.solar_charger.info",
          event_name: "cvr.solar_charger.info",
          description: "The solar charger controller infomation",
          measurement: :value,
          tags: [:vendor, :product_id, :firmware, :serial_number]
        ),
        last_value(
          "cvr.solar_charger.voltage",
          event_name: "cvr.solar_charger.voltage",
          description: "The controller main channel (or battery) voltage in mV",
          measurement: :value,
          tags: [:product_id, :serial_number]
        ),
        last_value(
          "cvr.solar_charger.current",
          event_name: "cvr.solar_charger.current",
          description: "The controller main channel (or battery) current in mA",
          measurement: :value,
          tags: [:product_id, :serial_number]
        ),
        last_value(
          "cvr.solar_charger.panel_voltage",
          event_name: "cvr.solar_charger.panel_voltage",
          description: "The solar panel voltage in mV",
          measurement: :value,
          tags: [:product_id, :serial_number]
        ),
        last_value(
          "cvr.solar_charger.panel_power",
          event_name: "cvr.solar_charger.panel_power",
  ...
  defp execute_solar_charger_voltage_event(data) do
    :telemetry.execute(
      [:tpro_nvr, :solar_charger, :voltage],
      %{value: data.v},
      %{
        product_id: data.pid,
        serial_number: data.serial_number
  ...
  defp execute_solar_charger_current_event(data) do
    :telemetry.execute(
      [:tpro_nvr, :solar_charger, :current],
      %{value: data.i},
      %{
        product_id: data.pid,
        serial_number: data.serial_number
  ...
  defp execute_solar_charger_panel_voltage_event(data) do
    :telemetry.execute(
      [:tpro_nvr, :solar_charger, :panel_voltage],
      %{value: data.vpv},
      %{
        product_id: data.pid,
        serial_number: data.serial_number
  ...
  defp execute_solar_charger_panel_power_event(data) do
    :telemetry.execute(
      [:tpro_nvr, :solar_charger, :panel_power],
      %{value: data.ppv},
      %{
        product_id: data.pid,
        serial_number: data.serial_number
