defmodule TProNVRWeb.API.DeviceStreamingController do
  @moduledoc false
        ...
        {:ok, path} when is_binary(path) ->
          case read_playlist(path) do
            {:ok, manifest_file} ->
              processed_content = 
                manifest_file
                |> remove_unused_stream(params)
                |> HLS.Processor.add_query_params(:playlist, query_params)
                |> HLS.Processor.add_query_params(:media_playlist, query_params)
              
              conn
              |> put_resp_content_type("application/vnd.apple.mpegurl")
              |> send_resp(200, processed_content)
              
            {:error, :enoent} ->
              conn
              |> put_resp_header("retry-after", "5")
              |> send_resp(503, "Stream not available yet. Please retry in a few seconds.")
          end
  ...
  # Try reading playlist - supports both naming conventions
  defp read_playlist(path) do
    index_path = Path.join(path, "index.m3u8")
    main_stream_path = Path.join(path, "main_stream.m3u8")
    
    Logger.info("[HLS] read_playlist checking path: #{path}")
    Logger.info("[HLS] index_path exists: #{File.exists?(index_path)}, main_stream_path exists: #{File.exists?(main_stream_path)}")
    
    cond do
      File.exists?(index_path) -> File.read(index_path)
      File.exists?(main_stream_path) -> File.read(main_stream_path)
      true -> {:error, :enoent}
    end
  end
  ...
  def hls_stream_segment(conn, %{"segment_name" => segment_name} = params) when not is_map_key(params, "stream_id") do
    ...
    {:ok, safe_segment_name} = Path.safe_relative(segment_name, base_path)
    full_path = Path.join(base_path, safe_segment_name)
  ...
  # Handle playback streaming segments (with stream_id for recorded footage)
  def hls_stream_segment(
    ...
    case File.exists?(full_path) do
      true ->
        if String.ends_with?(segment_name, ".m3u8") do
          TProNVRWeb.HlsStreamingMonitor.update_last_access_time(stream_id)

          content = File.read!(full_path)
          Logger.debug("[HLS] Raw playlist content:\n#{content}")
          
          processed = HLS.Processor.add_query_params(content, :media_playlist,
            stream_id: stream_id,
            live: params["live"]
          )
          
          Logger.debug("[HLS] Processed playlist:\n#{processed}")
  ...
  defp serve_snapshot_from_recorded_videos(conn, %{time: time} = params) do
    ...
    try do
      ...
      e in ErlangError ->
        Logger.warning("[Snapshot] Decode failed: #{inspect(e)}")
        {:error, :decode_failed}
    end
  end
  ...
  defp download_dir do
    ...
    Application.get_env(:tpro_nvr, :download_dir) || default_dir
  end
end
