defmodule TProNVRWeb.RecordingListLive do
  @moduledoc false

  require Logger
  use TProNVRWeb, :live_view

  import TProNVRWeb.ViewUtils
    ...
    <div class="grow e-m-8">
      ...
      >
        ...
        <:col :let={recording} label="Start-date" field={:start_date}>
          {format_date(recording.start_date, get_user_timezone(@current_user))}
        </:col>
        <:col :let={recording} label="End-date" field={:end_date}>
          {format_date(recording.end_date, get_user_timezone(@current_user))}
        </:col>
        <:action :let={recording}>
          <div class="flex justify-end">
            ...
            >
              <span title="Show information">
                <.icon
                  ...
                  class="w-6 h-6 mr-2 dark:text-white/80 cursor-pointer"
                />
              </span>
            </button>
            ...
            >
              <.icon
                name="hero-eye-solid"
                class="w-6 h-6 z-auto mr-2 dark:text-white/80 cursor-pointer thumbnail"
              />
            </span>
            <div class="flex justify-end">
              <.link
                href={~p"/api/devices/#{recording.device_id}/recordings/#{recording.filename}/blob"}
                class="inline-flex items-center text-white rounded-lg"
                id={"recording-#{recording.id}-link"}
              >
                <span title="Download recording">
                  <.icon name="hero-arrow-down-tray-solid" class="w-6 h-6 dark:text-white/80" />
                </span>
              </.link>
            </div>
          </div>
  ...
  def recording_details_popover(%{rec_details: rec_details} = assigns) do
    ...
    <div
      ...
      role="tooltip"
      class="absolute z-10 invisible inline-block w-64 text-sm text-white/60 transition-opacity duration-300 bg-black border border-green-700 rounded-lg shadow-sm opacity-0 dark:text-white/80 dark:border-green-700 dark:bg-black"
    >
      <div class="px-3 py-2 bg-green-900 border-b border-green-700 rounded-t-lg dark:border-green-700 dark:bg-black">
        <h3 class="font-semibold text-white dark:text-white">Recording Details</h3>
      </div>
  ...
  def get_user_timezone(nil), do: "Asia/Ho_Chi_Minh"
  def get_user_timezone(user), do: user.timezone || "Asia/Ho_Chi_Minh"
end
