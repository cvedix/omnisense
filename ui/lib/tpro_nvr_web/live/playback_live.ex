defmodule TProNVRWeb.PlaybackLive do
  ...
  def render(assigns) do
    ~H"""
    <div class="bg-black font-mono w-full h-full relative">
      <div :if={@devices == []} class="flex flex-col items-center justify-center h-full text-green-700 tracking-widest text-base uppercase">
        <div class="mb-4">
          <.icon name="hero-video-camera" class="w-16 h-16 opacity-50" />
        </div>
        <span>NO_ASSETS_DETECTED</span>
        <span><.link href={~p"/devices"} class="mt-4 px-4 py-2 border border-green-700 hover:border-green-500 hover:text-green-400 bg-green-900/20 transition-all shadow-inner block">PROVISION_DEVICES</.link></span>
      </div>
      <div :if={@devices != []} class="h-full relative shadow-[inset_0_0_50px_rgba(0,128,0,0.1)] border border-green-900/40">
        <div class="absolute top-0 right-0 w-4 h-4 border-t-2 border-r-2 border-green-500 z-10 pointer-events-none"></div>
        <div class="absolute bottom-0 left-0 w-4 h-4 border-b-2 border-l-2 border-green-500 z-10 pointer-events-none"></div>
        <.vue
          v-component="Viewer"
          class="e-h-full"
          segments={@segments}
      ...
      <.modal id="download-modal" class="backdrop-blur-sm bg-black/80">
        <div class="bg-black border border-green-500 shadow-[0_0_20px_rgba(34,197,94,0.3)] p-8 rounded-none relative">
          ...
          <h2 class="text-lg text-green-500 font-bold mb-6 tracking-widest uppercase border-b border-green-900 pb-2">DATA_EXFILTRATION_PROTOCOL</h2>
          <.simple_form
            for={@footage_form}
            id="footage_form"
  ...
  def handle_params(params, _uri, socket) do
    ...
    stream = Map.get(params, "stream", socket.assigns[:stream]) || "main_stream"
  ...
  def handle_event("switch_device", %{"device" => device_id}, socket) do
    route =
      Routes.playback_path(socket, :new, %{device_id: device_id, stream: socket.assigns.stream})
  ...
  def handle_event("switch_stream", %{"stream" => stream}, socket) do
    route =
      Routes.playback_path(socket, :new, %{
  ...
  defp assign_runs(socket) do
    ...
    # Keep segments in UTC - no timezone conversion for playback
    segments =
      Recordings.list_runs(%{device_id: device.id})
      |> Enum.map(fn run ->
        %{
          start_date: run.start_date |> DateTime.to_naive(),
          end_date: run.end_date |> DateTime.to_naive()
        }
      end)
  ...
  defp assign_timezone(socket) do
    user_tz = get_user_timezone(socket.assigns.current_user)
    assign(socket, timezone: user_tz)
  end

  defp get_user_timezone(nil), do: "Asia/Ho_Chi_Minh"
  defp get_user_timezone(user), do: user.timezone || "Asia/Ho_Chi_Minh"
  ...
  defp live_view_enabled?(socket) do
    ...
    enabled? =
      cond do
        ...
        not TProNVR.Utils.run_main_pipeline?() -> false
  ...
  # Treat timeline timestamps as UTC directly - no timezone conversion
  defp parse_datetime(datetime, _timezone) do
    # First try ISO8601 with timezone suffix
    case DateTime.from_iso8601(datetime) do
      {:ok, dt, _} -> dt
      _ ->
        # Try as naive datetime and treat as UTC
  ...
  defp format_date(datetime) do
    # Datetime is already in UTC from parse_datetime, just output ISO8601
