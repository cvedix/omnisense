defmodule TProNVR.MixProject do
  use Mix.Project
  ...
  defp deps do
    [
      ...
      {:rtsp, "~> 0.5.0"},
      # {:membrane_rtsp_plugin, "~> 0.7.1"}, # Incompatible API - needs full refactor
      {:ex_sdp, "~> 1.0"},
      ...
      {:membrane_h26x_plugin, "~> 0.10.0"},  # Now required for membrane_rtsp_plugin
      {:mimic, "~> 2.1", only: :test},
  ...
  defp generate_deb_package(release, {arch, _abi}) do
    ...
    if generate_deb? do
      name = "tpro_nvr_#{release.version}-1_#{get_debian_arch(arch)}"
      ...
      File.mkdir!(dest)
      File.mkdir_p!(Path.join([dest, "opt", "cvr"]))
      File.mkdir_p!(Path.join([dest, "var", "lib", "cvr"]))
      File.mkdir_p!(Path.join([dest, "usr", "lib", "systemd", "system"]))
      File.mkdir_p!(Path.join(dest, "DEBIAN"))

      File.cp_r!(release.path, Path.join([dest, "opt", "cvr"]))
      ...
      # systemd service
      dest = Path.join([dest, "usr", "lib", "systemd", "system", "tpro_nvr.service"])
      Path.join([templates_dir(), "debian", "tpro_nvr.service"]) |> File.cp!(dest)
  ...
  defp templates_dir(), do: :code.priv_dir(:tpro_nvr) |> List.to_string()
end
